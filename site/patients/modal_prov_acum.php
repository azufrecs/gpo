<?php
	...
	$userid = $_POST['userid'];
	$sql = $mysqli->query("SELECT provincia, municipio, Count(municipio) AS pacientes FROM tbl_pacientes GROUP BY provincia, municipio HAVING provincia='$userid' ORDER BY pacientes DESC");
	
	$Consulta = $mysqli->query("SELECT * FROM tbl_pacientes WHERE provincia = '$userid'");
